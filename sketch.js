...
function drawChart() {
  ...
  d3.csv("EAZ_Anzahl_Kurse.csv").then(raw => {
    ...
    function showTooltip(event, d) {
      ...
      tooltipText.attr("text-anchor", "start").attr("x", padX).attr("y", padY)
      ...
      // ⭐ Texte centré horizontalement dans la bulle, pas juste calé à gauche.
      tooltipText.attr("text-anchor", "middle").attr("x", boxW / 2);
    ...
    // --- Survol : contour marqué + année/valeur recentrées dans la case
    // + bulle avec la valeur exacte ---
    cell
      .on("mouseover", function (event, d) {
        ...
        const w = d.x1 - d.x0;
        const h = d.y1 - d.y0;
        const yearEl = d3.select(this).select("text.tile-year");
        const valueEl = d3.select(this).select("text.tile-value");
        const hasValue = !valueEl.empty();

        if (!yearEl.empty()) {
          yearEl.transition().duration(150)
            .attr("text-anchor", "middle")
            .attr("x", w / 2)
            .attr("y", hasValue ? h / 2 - 6 : h / 2 + 6);
        }
        if (hasValue) {
          valueEl.transition().duration(150)
            .attr("text-anchor", "middle")
            .attr("x", w / 2)
            .attr("y", h / 2 + 16);
        }
      ...
      .on("mouseout", function (event, d) {
        ...
        d3.select(this).select("text.tile-year")
          .transition().duration(150)
          .attr("text-anchor", "start")
          .attr("x", 8)
          .attr("y", 22);
        d3.select(this).select("text.tile-value")
          .transition().duration(150)
          .attr("text-anchor", "start")
          .attr("x", 8)
          .attr("y", 40);
